...
from AutoTransformPy import mirror as mir

def test_inputs():
    with pytest.raises(TypeError):
       mir.mirror("imgs/milad.jpg", 7) # direction must be string
       mir.mirror(True, "horizontal") # image path must be a string

    with pytest.raises(ValueError):
        mir.mirror("imgs/milad.jpg", "h") # Not a valid string option for direction
...
def test_return_imgs(): # Tests that the number of images returned from translate is correct
    ...
    returned_arr_1a = mir.mirror("imgs/milad.jpg", "horizontal") # should return 1 image
    returned_arr_1b = mir.mirror("imgs/milad.jpg", "vertical") # should return 1 image
    returned_arr_2a = mir.mirror("imgs/milad.jpg", "all") # should return 2 images
    returned_arr_2b = mir.mirror("imgs/milad.jpg") # all is default, should return 2 images
